...
def extract_all_content():
    ...
    for i, source in enumerate(st.session_state.sources):
        ...
        try:
            if source['type'] == 'url':
                content = extract_content(source['content'])
            elif source['type'] == 'text':
                content = extractor.extract_from_text(source['content'])
            elif source['type'] == 'pdf':
                content = extractor.extract_from_pdf(Path(source['content']))
            
            if content and content.is_valid:
                st.session_state.extracted_content.append(content)
            else:
                st.error(f"Failed to extract from source {i+1}: {content.error if content else 'Unknown error'}")
